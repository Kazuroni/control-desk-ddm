...
function CrossCheckTab() {
  ...
  const { data: sessions } = trpc.dashboard.getSessions.useQuery({
    reportType: "AgentDay",
  });
  ...
  const naoFiltrado = useMemo(() => {
    ...
    if (filterCampanha !== "all")
      d = d.filter(a => a.campanha === filterCampanha);
    if (filterSupervisorCC !== "all")
      d = d.filter((a: any) => a.supervisor === filterSupervisorCC);
    if (searchCC)
      d = d.filter(
        a =>
          a.agente?.toLowerCase().includes(searchCC.toLowerCase()) ||
          a.login?.toLowerCase().includes(searchCC.toLowerCase())
      );
    return d;
  }, [nao, filterCampanha, filterSupervisorCC, searchCC]);

  const allSelected =
    naoFiltrado.length > 0 && naoFiltrado.every((_, i) => selectedIds.has(i));
  ...
  function addToDimensionamento(a: {
    agente: string;
    login: string;
    campanha: string;
    uf: string;
  ...
  async function addSelectedInBatch() {
    ...
    const toastId = toast.loading(
      `Adicionando ${toAdd.length} operador(es)...`
    );
    let ok = 0;
    let fail = 0;
    ...
    if (fail === 0) {
      toast.success(
        `✓ ${ok} operador(es) adicionado(s) ao dimensionamento com sucesso!`,
        { duration: 4000 }
      );
    } else {
      toast.warning(
        `${ok} adicionado(s), ${fail} com erro. Verifique duplicatas.`,
        { duration: 5000 }
      );
    }
  }
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div className="rounded-xl border border-white/8 bg-white/3 p-4">
          ...
          <p className="text-2xl font-black text-white">
            {data?.totalDimensionamento ?? 0}
          </p>
          <p className="text-xs text-gray-500 mt-0.5">Cadastros ativos</p>
        </div>
        <div
          className={`rounded-xl border p-4 ${nao.length > 0 ? "border-orange-500/30 bg-orange-500/5" : "border-emerald-500/30 bg-emerald-500/5"}`}
        >
          <p className="text-xs text-gray-400 mb-1">Não Cadastrados</p>
          <p
            className={`text-2xl font-black ${nao.length > 0 ? "text-orange-400" : "text-emerald-400"}`}
          >
            {nao.length}
          </p>
      ...
      {nao.length === 0 ? (
        <div className="rounded-xl border border-emerald-500/20 bg-emerald-500/5 p-8 text-center">
          ...
          <p className="text-white font-semibold">
            Todos os agentes estão cadastrados!
          </p>
          <p className="text-gray-400 text-sm mt-1">
            Nenhum agente do AgentDay está fora do dimensionamento.
          </p>
        </div>
      ) : (
          ...
          <div className="px-4 py-3 bg-orange-500/5 border-b border-orange-500/15 space-y-3">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <span className="text-sm font-semibold text-orange-300">
                {nao.length} agente{nao.length !== 1 ? "s" : ""} sem cadastro no
                Dimensionamento
              </span>
              {selectedIds.size > 0 && (
                <Button
                ...
                >
                  ...
                  Adicionar {selectedIds.size} selecionado
            ...
            <div className="flex gap-2 flex-wrap">
              ...
              {supervisoresCC.length > 0 && (
                <Select
                  value={filterSupervisorCC}
                  onValueChange={setFilterSupervisorCC}
                >
                  <SelectTrigger className="w-44 h-8 text-xs bg-white/5 border-white/10">
            ...
            {naoFiltrado.length !== nao.length && (
              <p className="text-xs text-gray-500">
                Exibindo {naoFiltrado.length} de {nao.length} agentes
              </p>
            )}
          </div>
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-white/8 bg-white/3">
                ...
                <th className="text-left px-4 py-3 text-gray-400 font-medium text-xs">
                  Agente
                </th>
                <th className="text-left px-4 py-3 text-gray-400 font-medium text-xs">
                  Login
                ...
                <th className="text-center px-4 py-3 text-gray-400 font-medium text-xs">
                  Ação
                </th>
              </tr>
            </thead>
            <tbody>
              {naoFiltrado.length === 0 ? (
                ...
                naoFiltrado.map((a, i) => (
                  <tr
                    key={i}
                    className={`border-b border-white/5 hover:bg-white/3 ${selectedIds.has(i) ? "bg-orange-500/8" : i % 2 === 0 ? "" : "bg-white/2"}`}
                  >
                    ...
                    <td className="px-4 py-3 text-blue-300 text-xs font-medium">
                      {a.uf || "—"}
                    </td>
                    <td className="px-4 py-3 text-center">
                      <Button
...
export default function DimensionamentoPage() {
  ...
  const [viewMode, setViewMode] = useState<"tabela" | "celulas" | "cruzamento">(
    "tabela"
  );
  ...
  const { data: operadores = [], isLoading } =
    trpc.dimensionamento.list.useQuery({
  ...
  function exportExcel() {
    ...
    XLSX.writeFile(
      wb,
      `dimensionamento_${new Date().toISOString().slice(0, 10)}.xlsx`
    );
  ...
  const handleImportFile = useCallback(
    async (file: File) => {
      ...
      try {
        ...
        const res = await fetch("/api/upload-dimensionamento", {
          method: "POST",
          body: formData,
        });
        ...
        toast.success(
          `Importação concluída: ${json.inserted} inseridos, ${json.updated} atualizados — Performance em Tempo Real atualizada!`
        );
      } catch (e: any) {
  ...
  return (
    <div className="flex flex-col gap-6 p-4 md:p-6 min-h-screen">
      ...
      <div className="text-sm text-gray-400">
        ...
        encontrado{operadores.length !== 1 ? "s" : ""}
      </div>
      ...
      <Tabs value={viewMode} onValueChange={v => setViewMode(v as any)}>
        <TabsList className="bg-gray-900 border border-gray-800">
          ...
          <TabsTrigger
            value="cruzamento"
            className="data-[state=active]:bg-orange-500 data-[state=active]:text-white gap-1.5"
          >
              ...
              <div className="p-4 border-b border-gray-800 flex items-center gap-3">
                ...
                <div>
                  <div className="text-white font-bold">
                    DDM Control Desk — Quadro de Dimensionamento
                  </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-800 bg-gray-950">
                    <th className="text-left p-3 text-gray-400 font-medium whitespace-nowrap">
                      Nome
                    </th>
                    <th className="text-left p-3 text-gray-400 font-medium whitespace-nowrap">
                      Login
                    ...
                    <th className="text-right p-3 text-gray-400 font-medium whitespace-nowrap">
                      Ações
                    </th>
                  </tr>
                </thead>
                    ...
                    operadores.map((op, i) => (
                      ...
                      >
                        ...
                        <td className="p-3">
                          {op.turno ? (
                            <span
                              className={`px-2 py-0.5 rounded text-xs font-medium ${TURNO_COLORS[op.turno] || "bg-gray-700 text-gray-300"}`}
                            >
                              {op.turno}
                            </span>
                          ) : (
                        ...
                        <td className="p-3">
                          <span
                            className={`px-2 py-0.5 rounded-full text-xs font-medium border ${STATUS_COLORS[op.status || ""] || "bg-gray-700 text-gray-300 border-gray-600"}`}
                          >
                            {op.status || "—"}
                          </span>
                        </td>
                        <td className="p-3">
                          <div className="flex items-center justify-end gap-1">
                            <Button
                              variant="ghost"
                              size="icon"
                              className="w-7 h-7 text-gray-400 hover:text-blue-400"
                              onClick={() => openEdit(op)}
                            >
          ...
          <div className="flex flex-col gap-4">
            {isLoading ? (
              ...
              porCelula.map(([celula, ops]) => {
                const ativos = ops.filter(
                  o => o.status?.toUpperCase() === "ATIVO"
                ).length;
                ...
                const supervisoresCell = Array.from(
                  new Set(ops.map(o => o.supervisor).filter(Boolean))
                );
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <div className="text-xs text-gray-400">
                            {supervisoresCell.length > 0
                              ? `Sup: ${supervisoresCell.join(", ")}`
                              : "Sem supervisor"}
                          </div>
                        </div>
                      </div>
                      ...
                      <table className="w-full text-sm">
                        <thead>
                          <tr className="border-b border-gray-800">
                            <th className="text-left p-2 px-4 text-gray-500 font-medium text-xs">
                              Nome
                            </th>
                            <th className="text-left p-2 text-gray-500 font-medium text-xs">
                              Login
                            </th>
                            <th className="text-left p-2 text-gray-500 font-medium text-xs">
                              Supervisor
                            ...
                            <th className="text-right p-2 pr-4 text-gray-500 font-medium text-xs">
                              Ações
                            </th>
                          </tr>
                        </thead>
                        <tbody>
                          {ops.map((op, i) => (
                            <tr
                              key={op.id}
                              className={`border-b border-gray-800/40 hover:bg-gray-800/20 ${i % 2 === 0 ? "" : "bg-gray-900/20"}`}
                            >
                              <td className="p-2 px-4 text-white text-sm">
                                {op.nome}
                              </td>
                              ...
                              <td className="p-2">
                                {op.turno ? (
                                  <span
                                    className={`px-1.5 py-0.5 rounded text-xs ${TURNO_COLORS[op.turno] || "bg-gray-700 text-gray-300"}`}
                                  >
                                    {op.turno}
                                  </span>
                                ) : (
                              ...
                              <td className="p-2">
                                <span
                                  className={`px-1.5 py-0.5 rounded-full text-xs border ${STATUS_COLORS[op.status || ""] || "bg-gray-700 text-gray-300 border-gray-600"}`}
                                >
                                  {op.status || "—"}
                                </span>
                              </td>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 py-2">
            {[
              ...
              },
              {
                label: "Email",
                key: "email",
                placeholder: "joao.silva@grupoddm.com.br",
              },
              {
                label: "Supervisor",
                key: "supervisor",
              ...
              { label: "Célula", key: "celula", placeholder: "Ex: CRUZEIRO" },
              {
                label: "Skill",
                key: "skill",
                placeholder: "Ex: YDUQS/CRUZEIRO",
              },
              { label: "Cargo", key: "cargo", placeholder: "Ex: ATENDENTE" },
              {
                label: "Função",
                key: "funcao",
                placeholder: "Ex: OPERADOR DE TELEMARKETING ATIVO",
              },
              {
                label: "Departamento",
                key: "departamento",
            ...
            <div>
              ...
              <Select
                value={form.discador}
                onValueChange={v => setForm(f => ({ ...f, discador: v }))}
              >
                <SelectTrigger className="bg-gray-800 border-gray-700 text-white">
      ...
      <Dialog
        ...
        onOpenChange={v => {
          if (!importing) setImportDialogOpen(v);
        }}
      >
        <DialogContent className="max-w-lg bg-gray-950 border-gray-800">
          ...
          <div className="space-y-4">
            <p className="text-sm text-gray-400">
              ...
              <strong className="text-white">.xlsx</strong>. A planilha deve
              conter a aba <strong className="text-white">BaseQuadro</strong>{" "}
              com as colunas padrão. Operadores existentes serão{" "}
              <strong className="text-orange-300">atualizados</strong>, novos
              serão <strong className="text-green-300">inseridos</strong>.
            </p>
            <div
              className={`border-2 border-dashed rounded-xl p-8 text-center transition-colors cursor-pointer ${
                importing
                  ? "border-orange-500/50 bg-orange-500/5"
                  : "border-gray-700 hover:border-orange-500/60 hover:bg-orange-500/5"
              }`}
              ...
              <div className="rounded-xl border border-gray-800 bg-gray-900 p-4 space-y-3">
                ...
                <p className="text-[11px] text-gray-600">
                  ...
                  <strong className="text-gray-400">
                    {importResult.sheetName}
                  </strong>{" "}
                  · {importResult.totalRows} linhas processadas
                </p>
                  ...
                  <div className="rounded-lg bg-yellow-500/10 border border-yellow-500/20 p-3">
                    ...
                    <ul className="space-y-1">
                      {importResult.warnings.map((w, i) => (
                        <li key={i} className="text-[11px] text-yellow-300/80">
                          {w}
                        </li>
                      ))}
                    </ul>
      ...
      >
        <DialogContent className="bg-gray-900 border-gray-700 text-white max-w-sm">
          ...
          <p className="text-gray-400 text-sm">
            Tem certeza que deseja remover este operador do quadro de
            dimensionamento? Esta ação não pode ser desfeita.
          </p>
          <DialogFooter>
            <Button
              variant="outline"
