...
import PauseLimitsSettings from "@/components/PauseLimitsSettings";
...
const CustomTooltip = ({ active, payload, label }: any) => {
  ...
  return (
    <div className="bg-popover border border-border rounded-lg px-3 py-2 shadow-xl text-xs">
      <p className="font-semibold text-foreground mb-1 max-w-52 truncate">
        {label}
      </p>
      {payload.map((p: any) => (
...
export default function ReasonAgentPage() {
  ...
  const currentDateIndex =
    selectedDate && availableDates ? availableDates.indexOf(selectedDate) : 0;
  const canGoPrev = availableDates
  ...
  const abusadoresFiltrados = useMemo(() => {
    return abusadoresPausa
      ...
      .filter(
        (a: any) =>
          !filtroAgente ||
          a.agente.toLowerCase().includes(filtroAgente.toLowerCase())
      );
  ...
  const nr17Filtrados = useMemo(() => {
    return nr17Todos
      .filter(
        (a: any) => filtroNR17Motivo === "all" || a.motivo === filtroNR17Motivo
      )
      .filter(
        (a: any) =>
          !filtroNR17Agente ||
          a.agente.toLowerCase().includes(filtroNR17Agente.toLowerCase())
      );
  }, [nr17Todos, filtroNR17Motivo, filtroNR17Agente]);

  // Banheiro filtrado
  const banheiroFiltrado = useMemo(() => {
    return banheiroRanking.filter(
      (a: any) =>
        !filtroBanheiroAgente ||
        a.agente.toLowerCase().includes(filtroBanheiroAgente.toLowerCase())
    );
  ...
  const outrosFiltrados = useMemo(() => {
    return outrosRanking
      .filter(
        (a: any) =>
          filtroOutrosMotivo === "all" || a.motivo === filtroOutrosMotivo
      )
      .filter(
        (a: any) =>
          !filtroOutrosAgente ||
          a.agente.toLowerCase().includes(filtroOutrosAgente.toLowerCase())
      );
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">
            Controle de pausas, NR17, banheiro, feedback e outros ·{" "}
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <Button
            onClick={() =>
              exportPng(
                activeRef[activeTab] ?? exportRef,
                `tempos-${activeTab}-${selectedDate || new Date().toISOString().slice(0, 10)}.png`
              )
            }
            variant="outline"
            size="sm"
          ...
          <div ref={refGeralTab} className="space-y-5">
              ...
              {[
                {
                  ...
                  value: motivoChart.length,
                  color: "text-blue-400",
                  icon: <Activity className="w-4 h-4" />,
                },
                {
                  label: "Motivos Improdutivos",
                  value: motivoImprodChart.length,
                  color: "text-red-400",
                  icon: <AlertTriangle className="w-4 h-4" />,
                },
                {
                  label: "Agentes com Pausa Improd.",
                  value: agenteRanking.length,
                  color: "text-amber-400",
                  icon: <Clock className="w-4 h-4" />,
                },
                {
                  label: "Ofensores de Limite",
                  value: abusadoresPausa.length,
                  color: "text-red-500",
            ...
            <div className="bg-card border border-border rounded-xl p-5">
              <h3 className="text-sm font-semibold text-foreground mb-4 flex items-center gap-2">
                <Clock className="w-4 h-4 text-primary" /> Todos os Motivos de
                Pausa — Tempo Total
              </h3>
              {isLoading ? (
                ...
                <ResponsiveContainer
                  width="100%"
                  height={Math.max(200, motivoChart.slice(0, 15).length * 32)}
                >
                  <BarChart
                    data={motivoChart.slice(0, 15)}
                    layout="vertical"
                    ...
                    >
                      {motivoChart.slice(0, 15).map((entry: any, i: number) => (
                        <Cell
                          ...
                          fill={
                            entry.improdutiva
                              ? COLORS_IMPROD[i % COLORS_IMPROD.length]
                              : COLORS_GERAL[i % COLORS_GERAL.length]
                          }
                        />
                      ))}
                      <LabelList
            ...
            <div className="bg-card border border-border rounded-xl p-5">
              <h3 className="text-sm font-semibold text-foreground mb-1 flex items-center gap-2">
                <Clock className="w-4 h-4 text-amber-400" /> % Pausa Total por
                Agente
              </h3>
              <p className="text-xs text-muted-foreground mb-4">
                Tempo total em pausa — quanto mais alto, menos produtivo o
                agente
              </p>
              {isLoading ? (
                  ...
                  <table className="w-full text-xs">
                    <thead>
                      <tr className="border-b border-border">
                        ...
                        <th className="px-3 py-2 text-left text-muted-foreground font-medium w-40">
                          Barra
                        </th>
                      </tr>
                    </thead>
                    <tbody>
                      {pausaTotalPorAgente
                        ...
                        .map((r: any, i: number) => {
                          const maxSeg =
                            pausaTotalPorAgente[0]?.totalSegundos || 1;
                          const pct = Math.round(
                            (r.totalSegundos / maxSeg) * 100
                          );
                            ...
                            >
                              ...
                              <td className="px-3 py-2">
                                <div className="w-full bg-border/30 rounded-full h-2">
                                  <div
                                    className={`${severity} h-2 rounded-full transition-all`}
                                    style={{ width: `${pct}%` }}
                                  />
                                </div>
                              </td>
            ...
            <div className="bg-amber-500/10 border border-amber-500/30 rounded-xl p-4 flex gap-3">
              ...
              <div>
                <p className="text-sm font-semibold text-amber-400">
                  Pausas NR17 — Obrigatórias por Lei
                </p>
                <p className="text-xs text-muted-foreground mt-0.5">
                  Descanso 1 e 3:{" "}
                  <strong className="text-foreground">10 min</strong> · Lanche:{" "}
                  <strong className="text-foreground">20 min</strong> · Qualquer
                  segundo acima do limite já é considerado estourado.
                  <span className="text-amber-400 ml-1">
                    NR17 não é improdutiva, mas o excesso é monitorado.
                  </span>
                </p>
              </div>
            </div>
            ...
            <div className="flex gap-3 flex-wrap">
              <div className="relative flex-1 min-w-40">
                <Input
                  ...
                  value={filtroNR17Agente}
                  onChange={e => setFiltroNR17Agente(e.target.value)}
                  className="pl-3 bg-card border-border text-sm"
                />
              </div>
              <Select
                value={filtroNR17Motivo}
                onValueChange={setFiltroNR17Motivo}
              >
                ...
                <table className="w-full text-xs">
                  <thead>
                    <tr className="border-b border-border bg-muted/30">
                      <th className="px-3 py-3 text-left text-muted-foreground font-semibold uppercase tracking-wider">
                        #
                      </th>
                      <th className="px-3 py-3 text-left text-muted-foreground font-semibold uppercase tracking-wider">
                        Agente
                      ...
                      <th className="px-3 py-3 text-center text-muted-foreground font-semibold uppercase tracking-wider">
                        Limite
                      </th>
                      <th className="px-3 py-3 text-right text-muted-foreground font-semibold uppercase tracking-wider">
                        Tempo Usado
                      </th>
                      <th className="px-3 py-3 text-right text-muted-foreground font-semibold uppercase tracking-wider">
                        Nº Pausas
                  ...
                  <tbody>
                    {isLoading ? (
                      ...
                      <tr>
                        ...
                        >
                          Nenhum registro NR17 encontrado
                        </td>
                      </tr>
                    ) : (
                      nr17Filtrados.map((r: any, i: number) => {
                        const excedeuMin = r.excedeuLimite
                          ? Math.floor(r.excedidoSegundos / 60)
                          : 0;
                        const excedeuSec = r.excedeuLimite
                          ? r.excedidoSegundos % 60
                          : 0;
                        const cor = getNR17Color(r.motivo);
                        return (
                          <tr
                            key={`${r.agente}-${r.motivo}`}
                            className="border-b border-border/50 hover:bg-accent/20"
                          >
                            ...
                            <td className="px-3 py-2.5 text-right font-mono font-semibold">
                              {r.excedeuLimite ? (
                                <span className="text-red-400">
                                  +{excedeuMin}min{" "}
                                  {String(excedeuSec).padStart(2, "0")}s
                                </span>
                              ) : (
          ...
          <div ref={refBanheiroTab} className="space-y-5">
            <div className="bg-blue-500/10 border border-blue-500/30 rounded-xl p-4 flex gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-0.5">
                  Visão de tempo total e quantidade de ocorrências por agente.
                  Sem limite fixo, mas monitorado para identificar padrões de
                  abuso.
                </p>
              </div>
            </div>
            ...
            <div className="relative">
              <Input
                placeholder="Buscar agente..."
                value={filtroBanheiroAgente}
                onChange={e => setFiltroBanheiroAgente(e.target.value)}
                ...
                <table className="w-full text-xs">
                  <thead>
                    <tr className="border-b border-border bg-muted/30">
                      ...
                      <th className="px-3 py-3 text-left text-muted-foreground font-semibold uppercase tracking-wider w-36">
                        Volume
                      </th>
                    </tr>
                  </thead>
                    ...
                    {isLoading ? (
                      ...
                      <tr>
                        <td
                          ...
                          className="px-4 py-10 text-center text-muted-foreground"
                        >
                          Nenhum registro de banheiro encontrado
                        </td>
                      </tr>
                    ) : (
                      banheiroFiltrado.map((r: any, i: number) => {
                        ...
                        const mediaSeg =
                          r.totalPausas > 0
                            ? Math.round(r.totalSegundos / r.totalPausas)
                            : 0;
                        const alerta =
                          r.totalSegundos > 1800 || r.totalPausas > 5;
                        return (
                          <tr
                            key={r.agente}
                            className="border-b border-border/50 hover:bg-accent/20"
                          >
                            ...
                            <td
                              className={`px-3 py-2.5 text-right font-mono font-semibold ${alerta ? "text-amber-400" : "text-foreground"}`}
                            >
                              {secondsToHMS(r.totalSegundos)}
                            </td>
                            <td
                              className={`px-3 py-2.5 text-right ${r.totalPausas > 5 ? "text-amber-400 font-semibold" : "text-muted-foreground"}`}
                            >
                              {r.totalPausas}x
                            </td>
                            ...
                            <td className="px-3 py-2.5">
                              <div className="w-full bg-border/30 rounded-full h-2">
                                <div
                                  className={`${alerta ? "bg-amber-500" : "bg-blue-500"} h-2 rounded-full`}
                                  style={{ width: `${pct}%` }}
                                />
                              </div>
                            </td>
            ...
            <div className="bg-emerald-500/10 border border-emerald-500/30 rounded-xl p-4 flex gap-3">
              ...
              <div>
                <p className="text-sm font-semibold text-emerald-400">
                  Pausas de Feedback e Treinamento
                </p>
                <p className="text-xs text-muted-foreground mt-0.5">
                  Não são consideradas improdutivas. Visão de quem mais recebe
                  feedback/treinamento e por quanto tempo.
                </p>
              </div>
            </div>
                    ...
                    {isLoading ? (
                      ...
                      <tr>
                        <td
                          ...
                          className="px-4 py-10 text-center text-muted-foreground"
                        >
                          Nenhum registro de feedback/treinamento encontrado
                        </td>
                      </tr>
                    ) : (
          ...
          <div ref={refOutrosTab} className="space-y-5">
            <div className="bg-violet-500/10 border border-violet-500/30 rounded-xl p-4 flex gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-0.5">
                  Todos os motivos que não são NR17, banheiro ou feedback. Visão
                  por motivo + agente com tempo e ocorrências.
                </p>
              </div>
            </div>
            ...
            <div className="flex gap-3 flex-wrap">
              <div className="relative flex-1 min-w-40">
                <Input
                  placeholder="Buscar agente..."
                  value={filtroOutrosAgente}
                  onChange={e => setFiltroOutrosAgente(e.target.value)}
                  className="pl-3 bg-card border-border text-sm"
                />
              </div>
              <Select
                value={filtroOutrosMotivo}
                onValueChange={setFiltroOutrosMotivo}
              >
                ...
                <table className="w-full text-xs">
                  <thead>
                    <tr className="border-b border-border bg-muted/30">
                      ...
                      <th className="px-3 py-3 text-left text-muted-foreground font-semibold uppercase tracking-wider w-32">
                        Volume
                      </th>
                    </tr>
                  </thead>
                      ...
                      outrosFiltrados.map((r: any, i: number) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <td className="px-3 py-2.5">
                              <span className="px-2 py-0.5 rounded text-[10px] font-semibold bg-violet-500/15 text-violet-400">
                                {r.motivo}
                              </span>
                            </td>
                            <td className="px-3 py-2.5 text-right font-mono font-semibold text-violet-400">
                              {secondsToHMS(r.totalSegundos)}
                            </td>
                            <td className="px-3 py-2.5 text-right text-muted-foreground">
                              {r.totalPausas}x
          ...
          <div ref={refAbusadoresTab} className="space-y-5">
            <div className="bg-red-500/10 border border-red-500/30 rounded-xl p-4 flex gap-3">
              ...
              <div>
                <p className="text-sm font-semibold text-red-400">
                  Top 15 Ofensores de Pausa
                </p>
                <p className="text-xs text-muted-foreground mt-0.5">
                  Agentes que ultrapassaram o limite de qualquer pausa com
                  limite definido. Clique nos cabeçalhos para ordenar.
                </p>
              </div>
            </div>
            ...
            {(() => {
              const [sortCol, setSortCol] =
                useState<string>("excedidoSegundos");
              ...
              const sorted = [...abusadoresFiltrados]
                .sort((a: any, b: any) => {
                      ...
                      <tbody>
                        {isLoading ? (
                          ...
                          <tr>
                            ...
                            >
                              Nenhum ofensor encontrado com os filtros
                              selecionados.
                            </td>
                          </tr>
                        ) : (
                          sorted.map((r: any, i: number) => {
                            const excedeuMin = Math.floor(
                              r.excedidoSegundos / 60
                            );
                            const excedeuSec = r.excedidoSegundos % 60;
                            const limiteMin = r.limiteSegundos
                              ? Math.round(r.limiteSegundos / 60)
                              : null;
                            const pctExcesso = r.limiteSegundos
                              ? Math.round(
                                  (r.excedidoSegundos / r.limiteSegundos) * 100
                                )
                              : 0;
                            const gravidade =
                              pctExcesso > 50
                            ...
                            return (
                              ...
                              >
                                ...
                                <td className="px-3 py-2.5 text-center">
                                  <span
                                    className={`text-[10px] font-bold px-2 py-0.5 rounded ${gravidade.cls}`}
                                  >
                                    {gravidade.label}
                                  </span>
                                </td>
